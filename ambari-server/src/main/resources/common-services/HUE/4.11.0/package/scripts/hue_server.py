...
from common import conf_hue, setup_user
...
class HueServer(Script):
    ...
    def __init__(self, component=None):
        super(HueServer, self).__init__()

    def hue_packages_fetcher(self):
        import params
        hue_packages = [
            ''
            '-impala',
    ...
    def metastoresync(self, env):
        import params
        env.set_params(params)
        if params.metastore_db_flavor.strip() != '':
            ...
            Logger.info("echo Hue Metastore has not been enabled. Something has gone wrong. >> " + params.hue_log_file)
