...
hue_version = '4.11.0'
download_url = 'echo https://cdn.gethue.com/downloads/hue-4.11.0.tgz'
# can clean this one up using hue_version where this var is used
hue_version_dir = 'hue-4.11.0'
# New Cluster Stack Version that is defined during the RESTART of a Rolling Upgrade
version = default("/commandParams/version", None)
stack_name = default("/hostLevelParams/stack_name", None)
#e.g. /var/lib/ambari-agent/cache/stacks/ODP/$VERSION/services/HUE/package
...
#Fetch the current_stack version
command = ["/usr/bin/odp-select", "--version"]
# Run the command and capture the output
process = Popen(command, stdout=PIPE, stderr=PIPE)
output, error = process.communicate()
# Decode the output to a string
stack_version = output.decode('utf-8') if output else ''
stack_version = str(stack_version.strip())
...
hue_local_home_dir = '/home/{0}'.format(hue_user)
...
# ODP3 expects a / separated list of hosts
hbase_master_hosts = 'odp.cloudera.com/'
hbase_clusters = []
hbase_cluster = ''
...
# This needs to become a config option
spark_version_home = "spark2-client"
if "spark3-defaults" in config["configurations"]:
  spark_version_home = "spark3-client"
spark_home = "/usr/odp/current/"+spark_version_home
...
desktop_secret_key = config['configurations']['hue-desktop-site']['secret_key'].strip()
desktop_secret_key_script = config['configurations']['hue-desktop-site']['secret_key_script']

if len(desktop_secret_key) < 30 and desktop_secret_key_script.strip() == "":
  raise ValueError("Your secret key must be at least 30 characters long.\
  Using a known or weak secret key can lead to remote code execution vulnerabilities.")
